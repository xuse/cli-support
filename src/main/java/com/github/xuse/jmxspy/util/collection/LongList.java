package com.github.xuse.jmxspy.util.collection;

import java.util.Arrays;
import java.util.List;

/**
 * 提供和{@link List}接口相同的功能，但是免去了long类型的装箱和拆箱，从而得到性能上的改善。
 * 适用于大规模操作long基本类型数组的情况。
 * 
...
public final class LongList {
    ...
    */
     ...
     */
    public void addAll(int index, long[] in){
 ...
 public boolean retainAll(LongList c){
	LongList result=new LongList(Math.min(size, c.size));
 	for(int i=0;i<size;i++){
 		long value=list[i];
 		if(c.contains(value)){
 			result.add(value);
